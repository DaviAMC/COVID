...
@author: asaa, davi
"""
#vou fazer o gif para o novos casos
...
def write_analise(write_dict,update_date,gifs):#write_analise() missing 1 required positional argument: 'gifs'
    ...
    html_file.write('<div style="text-align: center;"><img style="width: 432px; height: 288px;" alt="Novos casos" src="'+gifs+regfile+"NC.gif"+'">&nbsp; <img style="width: 432px; height: 288px;" alt="Novas mortes" src="'+regfile+"NM.jpg"+'"><br></div>')
...
def drawNC(dR_raw,dR_smooth,reg,regfile,date,graf,gifs):#estou fazendo o gif aqui
#
# Gráfico Novos Casos  
#
    image = []
...
def drawNM(dD_raw,dD_smooth,reg,regfile,date):
    ...
    plt.savefig(regfile+"NM.jpg",bbox_inches="tight") 
...
def graf_estado(reg,linecsv,N_max,R_permil_global,D_permil_global,
...
#
    ...
    drawCA(R_raw,R_smooth4,regstr,regfile,date)
    drawMA(D_raw,D_smooth4,regstr,regfile,date)
    ...
    drawCAS(R_raw,D_raw,N_s,N_d,regstr,regfile,date)
    drawNM(dD_raw,dD_smooth4,regstr,regfile,date)
...
def graf_cidade(reg,linecsv,N_max,R_permil_global,D_permil_global,
                gamma1,gamma2,alpha,R0list,reglist,date,graf,gifs,
                dict_estados):
    print(reg)
    res = read_csv_data(reg,linecsv)
